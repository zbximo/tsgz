...
class DataEvent(Base):
    ...
    status = Column(VARCHAR(255), server_default=text("'1'"), comment='0已终止 1监测中')
...
class DataNew(Base):
    ...
    del_flag = Column(TINYINT, index=True, comment='0：未删除    1：删除')
    title = Column(TEXT, comment='标题')
    original_title = Column(TEXT, comment='原文标题')
    ...
    platform = Column(VARCHAR(255), index=True, comment='平台')
    ...
    screenshot_origin = Column(TEXT, comment='原始截图')
    images_origin = Column(TEXT, comment='原始图片')
    videos = Column(TEXT, comment='视频地址')
    video_cover_image = Column(TEXT, comment='媒体封面')
    duration = Column(TEXT, comment='时长')
    logo = Column(TEXT, comment='媒体logo')
    web_name = Column(TEXT, comment='站点名')
    domain = Column(TEXT, comment='域名')
    task_url = Column(VARCHAR(2550), comment='原任务url')
    is_google_add = Column(TINYINT, server_default=text("'0'"), comment='是否添加到谷歌新闻表')
...
class DataPerson(Base):
    ...
    uid = Column(VARCHAR(255), index=True, comment='人员id')
...
class DataSimilar(Base):
    ...
    id = Column(BigInteger, primary_key=True)
    plan_id = Column(BigInteger, index=True, comment='方案id')
    news_ids = Column(VARCHAR(500), index=True, comment='相似的新闻数组')
...
class DataSocialComment(Base):
    ...
    post_id = Column(VARCHAR(255), index=True, comment='帖子id')
    ...
    is_Translated = Column(Integer, server_default=text("'0'"), comment='是否翻译，翻译完请将此列置为1')
    comment_Time = Column(DateTime, comment='评论时间')
    add_Time = Column(DateTime, comment='添加时间')
    platform = Column(VARCHAR(255), comment='平台')
    comment_Avatar_origin = Column(TEXT, comment='评论头像')
    emotion = Column(Integer, index=True, comment='情感 0正面 1负面 2中立')
    comment_id = Column(VARCHAR(255), comment='评论id')
...
class DataSocialPost(Base):
    ...
    is_es_add = Column(TINYINT, comment='是否推入es，1是，0否')
    images = Column(TEXT, comment='图片  不用了 ')
    post_id = Column(VARCHAR(255), comment='post_id')
    screenshot_origin = Column(VARCHAR(255), comment='原始截图，现在已经没在用了')
    source_picture_list = Column(TEXT, comment='源图片链接')
    video_list = Column(TEXT, comment='视频封面链接')
    source_video_list = Column(TEXT, comment='源视频链接')
    duration = Column(TEXT, comment='视频时长')
    business_type = Column(VARCHAR(255), comment='业务类型，null普通、column栏目、keyword话题')
    keyword = Column(VARCHAR(255), comment='话题关键词')
    is_column_fill = Column(TINYINT, server_default=text("'0'"), comment='是否流入栏目列表，0否，1是')
    picture_list = Column(TEXT, comment='图片链接')
    video_cover_image = Column(VARCHAR(255), comment='视频封面截图  不用了')
...
class DataTask(Base):
    ...
    create_date = Column(DateTime, server_default=text("CURRENT_TIMESTAMP"), comment='创建时间')
    keywords = Column(VARCHAR(5000), comment='检索关键词')
    update_date = Column(DateTime, server_default=text("CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"), comment='修改时间')
